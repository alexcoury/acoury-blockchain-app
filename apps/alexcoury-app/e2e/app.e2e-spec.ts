...
describe('Starting tests for alexcoury-app', function() {
  ...
  it('website title should be alexcoury-app', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('alexcoury-app');
    })
  });

  it('network-name should be acoury-netowrk@0.0.1',() => {
    element(by.css('.network-name')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('acoury-netowrk@0.0.1.bna');
    });
  });

  it('navbar-brand should be alexcoury-app',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('alexcoury-app');
    });
  });
    ...
    it('AlexsAsset component should be loadable',() => {
      page.navigateTo('/AlexsAsset');
      browser.findElement(by.id('assetName'))
      .then((assetName) => {
        return assetName.getText();
      })
      .then((txt) => {
        expect(txt).toBe('AlexsAsset');
      });
    });

    it('AlexsAsset table should have 4 columns',() => {
      page.navigateTo('/AlexsAsset');
      element.all(by.css('.thead-cols th')).then(function(arr) {
        expect(arr.length).toEqual(4); // Addition of 1 for 'Action' column
      });
